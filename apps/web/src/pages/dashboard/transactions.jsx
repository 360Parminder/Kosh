...
export default function Transactions() {
    ...
    return (
        <div className="container mx-auto px-4 py-8 max-w-6xl"></div>
            <h1 className="text-3xl font-bold mb-6">Transactions</h1>
            
            {/* Summary Card */}
            <motion.div 
                className="bg-blue-100 rounded-lg shadow p-6 mb-8"
            ...
            >
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
                    <h2 className="text-xl font-bold text-blue-800">
                        Total: {accountInfo.currency} {totalTransactions}
                    </h2>
                    <p className="text-sm text-blue-700 mt-2 md:mt-0"></p>
                        Account Number: {accountInfo.accountNumber} | Currency: {accountInfo.currency}
                    </p>
                </div>
            </motion.div>
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4"></div>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Search by Payment Name</label>
                        <div className="relative">
                            <input
                                type="text"
                                className="block w-full pl-10 pr-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50"></thead>
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Payment Name</th>
                                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Payment Type</th>
                                <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Amount ({accountInfo.currency})</th>
                            </tr>
                        </thead>
                            ...
                            {filteredTransactions.length > 0 ? (
                                filteredTransactions.map((transaction) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{transaction.paymentName}</td>
